...
class ArcFaceClassifier(nn.Module):
    """
    Косинусный классификатор с angular margin.
    При обучении: cos(θ + m) для правильного класса → логиты × scale.
    При inference (labels=None): cos(θ) × scale.

    Стандартные параметры: scale=64.0, margin=0.5.
    Результат: [-18.3, -22.1, 51.4, -9.8, -31.2] — явный пик на правильном классе.
    """
    def __init__(
        self,
        in_features: int,
        ...
        # фиксированные константы для margin
        self.cos_m = torch.tensor(float(np.cos(margin)))
        self.sin_m = torch.tensor(float(np.sin(margin)))
        self.th    = torch.tensor(float(np.cos(np.pi - margin)))
        self.mm    = torch.tensor(float(np.sin(np.pi - margin) * margin))
    ...
    def forward(
        ...
        labels   : (B,) — нужны только при обучении
        returns  : (B, num_classes) логиты
        """
        feat_norm   = F.normalize(features, dim=1)
        weight_norm = F.normalize(self.weight, dim=1)

        # cos(θ) для всех классов
        cos_theta = feat_norm @ weight_norm.T           # (B, C)
        cos_theta = cos_theta.clamp(-1.0 + 1e-7, 1.0 - 1e-7)

        if labels is None:
            # inference — чистый косинус без margin
            return cos_theta * self.scale

        # ---------- добавляем angular margin к правильному классу ----------
        sin_theta   = torch.sqrt(1.0 - cos_theta ** 2)

        # cos(θ + m) = cos·cos_m − sin·sin_m
        cos_theta_m = (
            cos_theta * self.cos_m.to(features.device)
            - sin_theta * self.sin_m.to(features.device)
        )

        # численная стабильность при θ > π − m
        cos_theta_m = torch.where(
            cos_theta > self.th.to(features.device),
            cos_theta_m,
            cos_theta - self.mm.to(features.device),
        )

        one_hot = torch.zeros_like(cos_theta)
        one_hot.scatter_(1, labels.view(-1, 1), 1.0)

        # правильный класс: cos(θ+m)·s, остальные: cos(θ)·s
        logits = (
...
class AmazonDistilBERT(nn.Module):
    ...
    def forward(
        ...
        if self.use_arcface:
            # при обучении передаём labels → margin активен
            # при inference labels=None → чистый косинус
            logits = self.classifier(feat, labels=labels)
        else:
...
def train_amazon_model(
    ...
    # label_smoothing дополнительно улучшает разделимость
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y, _ in tqdm(trainloader, desc=f"[Train] Epoch {epoch}/{epochs}"):
            ...
            # передаём y → ArcFace margin активен при обучении
        ...
        print(
            f"Epoch {epoch}: loss={running_loss/len(trainloader):.4f}  "
...
def evaluate_model(model, dataloader, device=DEVICE) -> float:
    """
    Inference: labels=None → ArcFace без margin, чистые косинусные логиты × scale.
    """
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for x, y, _ in dataloader:
            x, y    = x.to(device), y.to(device)
            preds   = model(x, labels=None).argmax(1)
...
def collect_negative_scores(
    ...
    for x, y, _ in tqdm(safe_loader, desc="Collecting negative scores"):
        # inference: labels=None → логиты без margin
        logits = model(x.to(device), labels=None).cpu()
...
# ============================================================
# 9. SCORE-FEATURE DATASET
# ============================================================
class ScoreFeatureDataset(Dataset):
    """
    Каждый элемент: (cnn_scores, features, target_decoy_scores, label)
        cnn_scores         : (num_classes,) — логиты модели (ArcFace × scale)
...
def create_score_feature_dataset(
    ...
    for x, y, _ in tqdm(safe_loader, desc="Building ScoreFeatureDataset"):
        ...
        # inference: labels=None → логиты без margin
        logits, feats = model(x, labels=None, return_features=True)
...
# ============================================================
# 10. ЗАГРУЗКА СОХРАНЁННЫХ ДАТАСЕТОВ
# ============================================================
...
if __name__ == "__main__":
    model, score_datasets = main(train_model=True)
